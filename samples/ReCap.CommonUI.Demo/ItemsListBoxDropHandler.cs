...
// https://github.com/wieslawsoltes/Xaml.Behaviors/blob/release/11.3.6.6/samples/BehaviorsTestApplication/Behaviors/ItemsListBoxDropHandler.cs
namespace ReCap.CommonUI.Demo
{
    public class ItemsListBoxDropHandler : DropHandlerBase
    {
        private bool Validate<TViewModel, T>(ListBox listBox
        , DragEventArgs e
        ...
        where T : new()
        {
            if (sourceContext is not T sourceItem
                || targetContext is not TViewModel vm
            ...
            switch (e.DragEffects)
            {
                case DragDropEffects.Copy:
                {
                    if (bExecute)
                    {
                        var clone = new T() { Title = sourceItem.Title + "_copy" };
